 ...
 */
public class TigerChecksum extends Checksum {
	/** The block size. */
	...
	public void reset() {
		this.bufferOffset = 0;
		this.state[0] = 0x0123456789abcdefL;
		this.state[1] = 0xfedcba9876543210L;
		this.state[2] = 0xf096a5b4c3b2e187L;
		this.count = 0;
	}
	
	private void pass(final long[] abc, final long[] x, final long n) {
		long a, b, c, temp;
	...
	private void keySchedule(final long[] x) {
	...
	private void transform() {
		...
		for (int index = 0; index < x.length; index++) {
			x[index] = (((long) this.block[index * 8]) & 0xFF)
				| ((((long) this.block[(index * 8) + 1]) & 0xFF) << 8)
				| ((((long) this.block[(index * 8) + 2]) & 0xFF) << 16)
				| ((((long) this.block[(index * 8) + 3]) & 0xFF) << 24)
				| ((((long) this.block[(index * 8) + 4]) & 0xFF) << 32)
				| ((((long) this.block[(index * 8) + 5]) & 0xFF) << 40)
				| ((((long) this.block[(index * 8) + 6]) & 0xFF) << 48)
				| ((((long) this.block[(index * 8) + 7]) & 0xFF) << 56);
		}
	...
	private void finish() {
		...
		blockIndex = (int) (this.count % BLOCK_SIZE);
		// Padding
		this.block[blockIndex++] = (byte) 0x01;
		if (blockIndex > 56) {
	...
	public byte[] getDigest() {
		...
		for (int index = 0; index < this.state.length; index++) {
			digest[index * 8] = (byte) (this.state[index]);
			digest[(index * 8) + 1] = (byte) (this.state[index] >>> 8);
			digest[(index * 8) + 2] = (byte) (this.state[index] >>> 16);
			digest[(index * 8) + 3] = (byte) (this.state[index] >>> 24);
			digest[(index * 8) + 4] = (byte) (this.state[index] >>> 32);
			digest[(index * 8) + 5] = (byte) (this.state[index] >>> 40);
			digest[(index * 8) + 6] = (byte) (this.state[index] >>> 48);
			digest[(index * 8) + 7] = (byte) (this.state[index] >>> 56);
		}
		return digest;
	}
}
